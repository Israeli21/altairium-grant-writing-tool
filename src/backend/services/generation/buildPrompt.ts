...
export function buildPrompt({ context, section }: BuildPromptParams): BuildPromptResult {
  ...
  if (grant) {
    ...
    if (grant.grant_opportunity_url) {
      headerLines.push(`- Opportunity URL: ${grant.grant_opportunity_url}`)
    }
    if (grant.structure_type) {
      headerLines.push(`- Structure type: ${grant.structure_type}`)
    }
  } else {
